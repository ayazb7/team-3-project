  ...
  return (
    <>
      <div className="fixed z-50 flex flex-col w-full h-auto overflow-hidden shadow-2xl">
        ...
        `}>
          <div className="absolute inset-0 bg-black/60" onClick={() => setShowMobileMenu(false)}></div>
            ...
            {options.map((item, index) => {
              ...
              return (
                <div key={index} className="flex items-center relative z-10">
                  ...
                  >
                    <div className={`absolute left-0 top-1/2 -translate-y-1/2 h-8 bg-gradient-to-b from-[#ac1ec4] to-[#1c50fe] rounded-r-full transition-all duration-200 ${isActive ? 'w-1' : 'w-0 group-hover:w-1'}`}></div>
