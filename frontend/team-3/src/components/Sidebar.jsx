...
export default function Sidebar() {
    ...
    const SidebarContent = ({ showLogo = true, onItemClick }) => (
        <div className="flex flex-col h-full bg-sidebar text-sidebar-foreground relative overflow-hidden">
            ...
            {showLogo && (
                <div className={`flex items-center h-20 px-4 border-b border-sidebar-border relative z-10`}>
                    <img src={FlowStateLogo} alt="FlowState" />
                </div>
            )}

            <nav className={`flex-1 px-3 space-y-2 relative z-10 ${showLogo ? 'py-4' : 'py-6'}`}>
                {menuItems.map((item) => (
                    <MenuItem
                        key={item.id}
                        item={item}
            ...
            `}>
                <div className="flex gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-sidebar-foreground/60 truncate text-left">{email}</div>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    {/* <button className="flex items-center justify-center w-full h-8 text-sm rounded-md border border-sidebar-ring hover:bg-sidebar-accent hover:border-sidebar-accent-foreground transition-colors">
                     ...
                     <button className="flex items-center justify-center w-full h-9 text-sm rounded-md border border-sidebar-primary/40 bg-sidebar-primary/10 hover:bg-sidebar-primary hover:text-sidebar-primary-foreground hover:border-sidebar-primary transition-all cursor-pointer" onClick={handleLogout}>
                        Logout
                    </button>
                </div>
            </div>

            <div className={`hidden md:flex p-3 border-t border-sidebar-border bg-sidebar-accent/20 relative z-10 ${isCollapsed ? 'justify-center' : 'justify-end'}`}>
                 <button onClick={toggleDesktopSidebar} className="p-2 rounded-md hover:bg-sidebar-primary/20 hover:text-sidebar-primary transition-all cursor-pointer group z-10">
